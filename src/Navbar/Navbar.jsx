...
const Navbar = () => {
  const { revealMenu, setRevealMenu, openProfile, closeProfile } =
    useGlobalContext();
    ...
    <header className="nav-Header w-full h-[4.5rem] bg-slate-100 flex items-center sticky top-0 z-50 shadow-md">
      <nav className="nav-area w-full h-[4.5rem] bg-white flex items-center  md:justify-between md:flex">
        ...
        <div className="Logo-container  mx-3 flex-1 md:flex-9 md:mr-0 md:mx-[3rem]">
            <NavLink
              className="logo w-[4rem] flex flex-col "
               to="/"
            >
              <span className="font-mono font-extrabold  text-center">
                Autumn
              </span>
              <span className="font-mono font-extralight text-center">
                blog
              </span>
            </NavLink>
        </div>
          ...
          {Data.map((menu, index) => {
            ...
            return (
              <button className="nav-item active-nav mx-3" key={id}>
                <NavLink className="nav-list" to={Path} key={Path}>
                  {Menu}
                </NavLink>
              </button>
            );
        ...
        <article className="profile-search  flex  ">
          <div className="search-bar w-[2.5rem] h-[2.5rem ] text-3xl font-normal  items-center justify-center cursor-pointer mx-3 hidden md:flex ">
            <BiSearch className="" />
          </div>
          <Profile />
        </article>
        {/* social media area */}
        <article className="socials w-fit conent h-full items-center hidden md:flex mr-5 ">
          {/* Icon area */}
          <div className="icon-container  h-full w-fit content items-center hidden md:flex">
            {Data.map((icons) => {
              const { Icon, id } = icons;
              return (
